...
app.post('/pair', async (req, res) => {
  ...
  if (!phone || !/^\d{7,15}$/.test(phone)) {
    return res.status(400).json({ error: 'Invalid phone number. Use country code without + (e.g. 2547XXXXXXXX)' })
  }
  ...
  try {
    ...
    res.status(500).json({ error: e.message || 'Failed to generate pairing code. Try again.' })
  }
})
